      ...
      [
        ...new Map(
          subjects
            ...
            .map((s) => [
              s.ClassID,
              { id: s.ClassID, label: s.ClassName || `Class ${s.ClassID}` },
            ]),
        ).values(),
  ...
  const filtered = useMemo(() => {
    ...
    return teachers.filter((teacher) => {
      ...
      if (filters.classId) {
        ...
        const classLabel = classOptions.find(
          (c) => String(c.id) === filters.classId,
        )?.label;
        if (classLabel) {
          const teacherSubjectKey = (teacher.Subject || "").toLowerCase();
          const classesForSubject = subjectClassMap[teacherSubjectKey];
          if (!classesForSubject || !classesForSubject.has(classLabel))
            return false;
        }
      }
