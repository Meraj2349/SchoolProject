  ...
  return (
    ...
    >
        ...
        <div className="relative w-[88px] h-[88px] mx-auto z-[1]">
          {showImage ? (
            <Image
              ...
              style={{ border: `3px solid ${GOLD}`, boxShadow: "0 4px 16px rgba(0,0,0,0.25)" }}
      ...
      <div className="px-5.5 py-5" style={{ padding: "20px 22px" }}>
        <h3 className="font-bold text-[1.15rem] text-center m-0 mb-3.5" style={{ color: NAVY }}>
          {teacher.FirstName} {teacher.LastName}
        </h3>
        <div className="flex flex-col gap-2 mb-3.5">
          {[
            { value: teacher.Subject, cls: "font-bold text-[0.9rem]", style: { color: NAVY } },
            { value: teacher.Email, cls: "text-gray-500 text-[0.88rem] break-all" },
            { value: teacher.ContactNumber, cls: "text-gray-500 text-[0.88rem]" },
          ].map((item, i) =>
            item.value ? (
              <div key={i} className="flex items-center gap-2.5 px-3 py-2 bg-gray-50 rounded-lg border border-gray-200 transition-colors hover:bg-[#eef0f8]">
                <span className={item.cls} style={item.style}>{item.value}</span>
              </div>
            ) : null
          )}
        </div>
...
export default function TeacherListPage() {
  ...
  const filtered = teachers.filter((teacher) => {
    ...
    if (filterBy === "subject") return teacher.Subject?.toLowerCase().includes(s);
    if (filterBy === "email") return teacher.Email?.toLowerCase().includes(s);
    return full.includes(s) || teacher.Subject?.toLowerCase().includes(s) || teacher.Email?.toLowerCase().includes(s);
  });
    ...
    return (
      <div className="min-h-screen bg-[#f0f2f8] font-sans">
        ...
        <div className="text-center px-6 py-12 bg-white rounded-2xl shadow-lg max-w-[500px] mx-auto mt-10 border border-gray-200">
          <p className="text-red-700 text-[1.1rem] mb-5">{t("failedToFetch")}</p>
          <button
            onClick={refetch}
  ...
  return (
    <div className="min-h-screen bg-[#f0f2f8] font-sans">
      ...
      <NavyHeader>
        <h1 className="relative z-[1] text-white font-extrabold m-0 mb-2 tracking-tight" style={{ fontSize: "clamp(1.8rem, 4vw, 2.8rem)", letterSpacing: "-0.02em" }}>
          {t("pageTitle")}
        </h1>
      ...
      <div
        ...
        style={{ padding: "22px 28px", borderTop: `3px solid ${GOLD}`, boxShadow: "0 6px 24px rgba(26,39,68,0.12)" }}
      >
        {[
          {
          ...
          <div key={i} className="flex flex-col gap-1.5">
            <label className="text-xs font-bold uppercase tracking-[0.06em]" style={{ color: NAVY }}>{field.label}</label>
            <select
              value={field.value}
              onChange={field.onChange}
              disabled={field.disabled}
              style={{ ...selectStyle, opacity: field.disabled ? 0.5 : 1, cursor: field.disabled ? "not-allowed" : "pointer" }}
      ...
      {/* Search section */}
      <div className="max-w-[920px] mx-auto my-7 bg-white px-7 py-6 rounded-2xl border border-gray-200" style={{ boxShadow: "0 6px 24px rgba(26,39,68,0.1)" }}>
        ...
        <div className="text-right pt-2.5 border-t border-gray-200">
          <span className="text-[0.82rem] font-bold px-3.5 py-1.5 rounded-full inline-block" style={{ background: NAVY, color: GOLD_LIGHT }}>
            {filtered.length} {filtered.length !== 1 ? t("teachersFound") : t("teacherFound")}
          </span>
        </div>
      </div>
      ...
      {/* Grid or empty state */}
      {filtered.length === 0 ? (
        <div className="text-center px-6 py-12 bg-white rounded-2xl shadow-md mx-auto mb-12 max-w-[480px] border border-gray-200" style={{ margin: "0 auto 48px" }}>
          <div className="text-[3.5rem] mb-3.5 opacity-55">🔍</div>
          <h3 className="font-semibold text-[1.2rem] m-0" style={{ color: NAVY }}>{t("noTeachersFound")}</h3>
        </div>
      ) : (
        <div className="grid gap-6 max-w-[1300px] mx-auto mb-12 px-6" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))" }}>
