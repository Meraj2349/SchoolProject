...
export default function GalleryPage() {
  ...
  const filtered =
    category === "all" ? images : images.filter((i) => i.ImageType === category);
  ...
  const catLabel = (type) => CATEGORIES.find((c) => c.value === type)?.label ?? type;
  const catIcon  = (type) => CATEGORIES.find((c) => c.value === type)?.icon ?? "";
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="absolute inset-0 opacity-[0.07]" style={{ backgroundImage: "radial-gradient(circle at 20% 50%, white 0%, transparent 60%), radial-gradient(circle at 80% 20%, white 0%, transparent 55%)" }} />
        <h1 className="relative z-[1] font-extrabold text-white m-0 text-shadow" style={{ fontSize: "clamp(1.75rem, 4vw, 2.75rem)" }}>
          {t("title")}
        </h1>
      </section>
      ...
      <section className="py-8">
        <div className="max-w-[1400px] mx-auto px-8">
          ...
          >
            <h2 className="text-[1.8rem] font-medium mb-6 text-gray-700">{t("browseByCategory")}</h2>
              ...
              {CATEGORIES.map((c) => (
                <button
                  ...
                  style={category === c.value
                    ? { background: "linear-gradient(135deg, #10b981, #059669)", boxShadow: "0 6px 20px rgba(16,185,129,0.4)" }
                    : {}}
                >
          ...
          {isError && (
            <div className="flex justify-center my-8">
              <div className="flex items-center gap-4 px-8 py-4 bg-red-50 border border-red-200 rounded-xl text-red-700 font-medium">
                ...
                <button onClick={refetch} className="px-4 py-2 bg-red-700 text-white border-none rounded-lg cursor-pointer font-medium hover:bg-red-800">
                  {t("tryAgain")}
                </button>
              </div>
            </div>
            ...
            <>
              ...
              >
                <p className="m-0 text-[1.1rem] text-gray-700">
                  {t("showing")} <strong>{filtered.length}</strong> {t("images")}
                  {category !== "all" && <> {t("in")} <strong>{catLabel(category)}</strong></>}
                </p>
              </motion.div>
                    ...
                    >
                      ...
                      <div className="relative w-full h-[250px] overflow-hidden">
                        ...
                        <div className="hidden absolute inset-0 flex-col items-center justify-center bg-gray-100 text-gray-500">
                          ...
                          <p className="m-0 text-sm">{t("imageNotAvailable")}</p>
                        </div>
                        {/* Hover overlay */}
                        <div
                        ...
                        >
                          <div className="text-center text-white translate-y-5 group-hover:translate-y-0 transition-transform duration-300">
                            ...
                            <p className="m-0 text-[1.1rem] font-medium">{t("viewImage")}</p>
                          </div>
                        </div>
                      </div>
                      {/* Info */}
                      <div className="p-6">
                        <div className="mb-3">
                          <span
                            ...
                            style={{
                              background: CAT_BADGE_STYLES[img.ImageType]?.bg ?? "#f8f9fa",
                              color: CAT_BADGE_STYLES[img.ImageType]?.color ?? "#495057",
                            }}
                          >
                        ...
                        {img.Description && (
                          <p className="m-0 mb-3 text-gray-600 leading-relaxed text-[0.95rem]">{img.Description}</p>
                        )}
                      </div>
                    </motion.div>
                  ))}
      ...
      {selected && (
        ...
        >
          <div className="relative max-w-[85vw] max-h-[85vh] flex items-center justify-center mx-20" onClick={(e) => e.stopPropagation()}>
            {/* Close */}
            <button
            ...
            >
              ...
              <div className="bg-white/95 p-6 rounded-xl mt-4 text-center backdrop-blur-[10px] max-w-[600px]">
                <span
                  ...
                  style={{
                    background: CAT_BADGE_STYLES[selected.ImageType]?.bg ?? "#f8f9fa",
                    color: CAT_BADGE_STYLES[selected.ImageType]?.color ?? "#495057",
                  }}
                >
                  {catIcon(selected.ImageType)} {catLabel(selected.ImageType)}
                </span>
                {selected.Description && (
                  <p className="text-[1.1rem] m-0 mb-4 text-gray-600 leading-relaxed">{selected.Description}</p>
                )}
