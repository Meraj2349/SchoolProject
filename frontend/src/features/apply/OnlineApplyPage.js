...
export default function OnlineApplyPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSubmitError(err?.response?.data?.message || err.message || t("submitFailed"));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      {success ? (
        <div className="max-w-[540px] mx-auto my-12 bg-white rounded-xl shadow-lg p-12 text-center">
          ...
          <h2 className="text-[1.6rem] font-bold text-green-800 m-0 mb-3">{t("successTitle")}</h2>
          <p className="text-gray-600 text-base m-0 mb-6 leading-relaxed">{t("successMessage")}</p>
          <button
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-5">
              ...
              <div className="sm:col-span-2 flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="applicant_name">
                  {t("applicantName")} <span className="text-red-500 ml-0.5">*</span>
                </label>
                <input
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="date_of_birth">
                  {t("dateOfBirth")} <span className="text-red-500 ml-0.5">*</span>
                </label>
                <input
              ...
              {/* Applying for class */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="applying_for_class">
                  {t("applyingForClass")} <span className="text-red-500 ml-0.5">*</span>
                </label>
                <input
                  id="applying_for_class" name="applying_for_class" type="text"
                  className={inputCls(errors.applying_for_class)}
                  value={form.applying_for_class} onChange={handleChange}
                />
              </div>

              {/* Previous school */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="previous_school">
                  {t("previousSchool")}
                </label>
                <input
                  id="previous_school" name="previous_school" type="text"
                  className={inputCls(false)}
                  value={form.previous_school} onChange={handleChange}
                />
              </div>

              {/* Previous class */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="previous_class">
                  {t("previousClass")}
                </label>
                <input
                  id="previous_class" name="previous_class" type="text"
                  className={inputCls(false)}
                  value={form.previous_class} onChange={handleChange}
                />
              </div>
              ...
              {/* Parent name */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="parent_name">
                  {t("parentName")} <span className="text-red-500 ml-0.5">*</span>
                </label>
                <input
              ...
              {/* Parent contact */}
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="parent_contact">
                  {t("parentContact")} <span className="text-red-500 ml-0.5">*</span>
                </label>
                <input
                  id="parent_contact" name="parent_contact" type="tel"
                  className={inputCls(errors.parent_contact)}
                  value={form.parent_contact} onChange={handleChange}
                />
              </div>

              {/* Parent email — full width */}
              <div className="sm:col-span-2 flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="parent_email">
                  {t("parentEmail")}
                </label>
                <input
                  id="parent_email" name="parent_email" type="email"
                  className={inputCls(false)}
                  value={form.parent_email} onChange={handleChange}
                />
              </div>
              ...
              <div className="sm:col-span-2 flex flex-col gap-1.5">
                <label className="text-sm font-semibold text-gray-700" htmlFor="additional_info">
                  {t("additionalInfo")}
                </label>
                <textarea
