...
export default function EventsPage() {
    ...
    const tick = () => {
      ...
      events.forEach((e) => {
        ...
        if (diff > 0) {
          ...
          next[e.EventID] = { days: 0, hours: 0, minutes: 0, seconds: 0, isExpired: true };
        }
      });
      setCountdowns(next);
    };
  ...
  function EventCard({ event, expired = false }) {
    ...
    return (
      ...
      >
        {today && (
          <div className="inline-flex items-center gap-1.5 text-white text-[11px] font-bold tracking-[0.08em] uppercase px-2.5 py-1 rounded-full w-fit"
        ...
        <div className="flex items-start justify-between gap-3 flex-wrap">
          ...
          <div className="text-xs text-gray-500 font-medium text-right leading-snug">
            ...
            {event.StartDate !== event.EndDate && <> – {fmtDate(event.EndDate)}</>}
          </div>
        </div>
        <div className="flex-1 flex flex-col gap-2">
          <h3 className="text-[1.1rem] font-bold text-gray-900 leading-snug m-0">{event.EventName}</h3>
          {event.Venue && <div className="text-xs text-gray-500">📍 {event.Venue}</div>}
          {event.Description && <p className="text-sm text-gray-600 leading-relaxed m-0">{event.Description}</p>}
          {cd && !cd.isExpired && (
            <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-3.5 mt-2 border border-green-200">
              <h4 className="text-[11px] font-bold uppercase tracking-[0.08em] text-green-900 m-0 mb-2.5">{t("endsIn")}</h4>
              <div className="grid grid-cols-4 gap-2">
                {COUNTDOWN_UNITS.map(({ key, labelKey }) => (
                  <div key={key} className="bg-white rounded-lg p-2 text-center shadow-sm">
                    <span className="block text-2xl font-extrabold text-emerald-600 leading-none tabular-nums">{cd[key]}</span>
                    <span className="block text-[10px] text-gray-500 font-semibold uppercase tracking-wide mt-0.5">{t(labelKey)}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="max-w-[1200px] mx-auto px-5 py-14 pb-20">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4 min-h-[40vh] text-center">
            ...
            <h3 className="text-xl font-bold text-gray-700 m-0">{t("errorLoadingEvents")}</h3>
            <button
              onClick={refetch}
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-4 min-h-[40vh] text-center">
            ...
            <h3 className="text-xl font-bold text-gray-700 m-0">{t("noUpcomingEvents")}</h3>
            <p className="text-gray-500 text-sm m-0">{t("checkBackLater")}</p>
          </div>
        ) : (
          <div>
            {events.length > 0 && (
              <h2 className="flex items-center gap-3 font-extrabold text-gray-900 mb-8" style={{ fontSize: "clamp(1.5rem, 3vw, 2rem)" }}>
                <span className="inline-block w-[5px] h-7 rounded-sm flex-shrink-0" style={{ background: "linear-gradient(180deg, #10b981, #059669)" }} />
            ...
            {pastEvents.length > 0 && (
              <>
                <h2 className="flex items-center gap-3 font-extrabold text-gray-900 mt-8 mb-8 opacity-70" style={{ fontSize: "clamp(1.5rem, 3vw, 2rem)" }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[repeat(auto-fill,minmax(340px,1fr))] gap-6 opacity-65">
                  {pastEvents.map((event) => <EventCard key={event.EventID} event={event} expired />)}
                </div>
              </>
            )}
          </div>
